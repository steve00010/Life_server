...
_serverSettingsConfig = configFile >> "CfgEpochServer"; {
    _selectedConfig = _serverSettingsConfig >> _x select 0;
    _defaultVarData = _x select 1;
    _varData =
        switch (true) do {
            case (isNumber _selectedConfig):
                {
                    getNumber _selectedConfig
                };
            case (isArray _selectedConfig):
                {
                    getArray _selectedConfig
                };
            case (isText _selectedConfig):
                {
                    getText _selectedConfig
                };
            default {
                _defaultVarData
            };
        };
        if (_varData isEqualTo "true") then {
            _varData = true
        };
    if (_varData isEqualTo "false") then {
        _varData = false
    };
    if (typeName _defaultVarData != typeName _varData) then {
        diag_log format["EPOCH-CONFIG: Error incorrect data type for %1 reverted to default (required: %2 provided: %3)", _x select 0, typeName _defaultVarData, typeName _varData];
        _varData = _defaultVarData;
    };
...
if (!isNil "EPOCH_allowedVehiclesList") then {
    _vehicleSlotLimit = 0; {
        _vehicleSlotLimit = _vehicleSlotLimit + (_x select 1)
    }
