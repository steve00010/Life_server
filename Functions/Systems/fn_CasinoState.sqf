...
switch(_state) do
{
	...
	case 0:
	{
		[[2,"The Casino robbery failed for unknown reasons."],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
		_vault setVariable["Casino_Robbed",true,true];
		sleep (10 * 60); //Wait 10 minutes
		_vault setVariable["Casino_Robbed",false,true];
		_vault setVariable["fed_rob_ip",false,true];
	};
	
	case 1:
	{
		[[1,format["$%1 was stolen from the Rebel Casino!",[_funds] call life_fnc_numberText]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
		_vault setVariable["Casino_Robbed",true,true];
		...
		[[_funds],"life_fnc_CasinoRaidSuccess",true,false] spawn life_fnc_MP;
	};
};