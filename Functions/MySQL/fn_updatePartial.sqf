...
switch(_mode) do {
	...
	case 2: {
		...
		switch(_side) do {
			...
			case east: {_query = format["UPDATE players SET arc_licenses='%1' WHERE playerid='%2'",_value,_uid];};			
		};
	};
	...
	case 3: {
		...
		switch(_side) do {
			...
			case east: {_query = format["UPDATE players SET arc_gear='%1' WHERE playerid='%2'",_value,_uid];};			
		};
	};
