...
if !(isNil "EPOCH_unsupportedCheckRunning") exitWith {
    diag_log "UnsupportedCheck: Already running aborted"
};
    ...
    if (!isNull _x) then {
        ...
        if (isText(_config)) then {
            ...
            if (!_isSupported) then {
                _isSupported =
                    if (terrainIntersectASL[_objectPos, [_objectPos select 0, _objectPos select 1, (_objectPos select 2) - 1]]) then {
                        true
                    } else {
